...
if [ -z "$REQUEST" ]; then
  echo "[ANALYSIS WORKER] ERROR: empty request"
  exit 1
fi
...
AGENT_ID="$(awk -F'|' '$1!~/^#/ && $1!="" && $4=="analysis"{print $1; exit}' workers/takomachi_agents.conf)"
if [ -z "$AGENT_ID" ]; then
  echo "[ANALYSIS WORKER] ERROR: no agent with capability 'analysis' in workers/takomachi_agents.conf"
  exit 1
fi
...
if [ -z "$TAKOMACHI_API_KEY" ]; then
  echo "[ANALYSIS WORKER] ERROR: could not retrieve TAKOMACHI_API_KEY from Keychain"
  exit 1
fi
...
if [ "$status" != "201" ]; then
  echo "[ANALYSIS WORKER] ERROR: task submission failed (HTTP $status)"
  exit 1
fi
...
while [ "$SECONDS" -lt "$DEADLINE" ]; do
  ...
  if [ "$status" != "200" ]; then
    echo "[ANALYSIS WORKER] ERROR: task fetch failed (HTTP $status)"
    exit 1
  fi
...
if [ "$TASK_STATUS" != "completed" ]; then
  echo "[ANALYSIS WORKER] ERROR: task did not complete in time (status=${TASK_STATUS:-timeout})"
  exit 1
fi
